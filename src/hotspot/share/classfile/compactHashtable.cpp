...
void CompactHashtableWriter::allocate_table() {
  ...
  _compact_buckets_size = _num_buckets + 1;
  _compact_buckets = (u4*)ArchiveBuilder::ro_region_alloc(sizeof(u4) * _compact_buckets_size);
  _compact_entries_size = entries_space;
  _compact_entries = (u4*)ArchiveBuilder::ro_region_alloc(sizeof(u4) * _compact_entries_size);

  _stats->bucket_count    = _num_buckets;
  _stats->bucket_bytes    = align_up(_compact_buckets_size * sizeof(u4),
                                     SharedSpaceObjectAlignment);
  _stats->hashentry_count = _num_entries_written;
  _stats->hashentry_bytes = align_up(_compact_entries_size * sizeof(u4),
                                     SharedSpaceObjectAlignment);
}
...
void CompactHashtableWriter::dump_table(NumberSeq* summary) {
  ...
  for (int index = 0; index < _num_buckets; index++) {
    ...
    int bucket_size = bucket->length();
    if (bucket_size == 1) {
      _compact_buckets[index] = BUCKET_INFO(offset, VALUE_ONLY_BUCKET_TYPE);

      Entry ent = bucket->at(0);
      // bucket with one entry is value_only and only has the encoded_value
      _compact_entries[offset++] = ent.encoded_value();
      ...
      _compact_buckets[index] = BUCKET_INFO(offset, REGULAR_BUCKET_TYPE);

      for (int i=0; i<bucket_size; i++) {
        Entry ent = bucket->at(i);
        _compact_entries[offset++] = u4(ent.hash());      // write entry hash
        _compact_entries[offset++] = ent.encoded_value(); // write entry encoded_value
      }
      if (bucket_size == 0) {
        _num_empty_buckets++;
      } else {
  ...
  _compact_buckets[_num_buckets] = BUCKET_INFO(offset, TABLEEND_BUCKET_TYPE);
  assert(offset == (u4)_compact_entries_size, "sanity");
}
