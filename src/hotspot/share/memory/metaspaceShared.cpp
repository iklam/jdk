...
char* MetaspaceShared::reserve_address_space_for_archives(FileMapInfo* static_mapinfo,
  ...
  const bool use_large_space = false;
  const bool split_reserved_memory = false; // never call os::split_reserved_memory
  if (MetaspaceShared::use_windows_memory_mapping() && use_archive_base_addr) {
    assert(base_address != NULL, "sanity");

    assert(total_range_size > ccs_begin_offset, "must be");
    address ccs_base = base_address + ccs_begin_offset;

    archive_space_rs = ReservedSpace(archive_space_size, archive_space_alignment,
                                     use_large_space, (char*) base_address);
    class_space_rs = ReservedSpace(class_space_size, class_space_alignment,
                                   use_large_space, (char*) ccs_base);
    ...
    // Now divide the space into ccs and cds archive. For simplicity, just leave
