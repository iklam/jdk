    ...
    if (DumpSharedSpaces) {
      ...
      if (archives == 1) {
        ...
        if (!success) {
          ...
          SharedArchivePath = (char*)SharedArchiveFile; // FIXME cast
        } else {
          // User has specified a single archive, which is a dynamic archive.
          SharedDynamicArchivePath = (char*)SharedArchiveFile; // FIXME cast
        ...
        if (SharedArchivePath == NULL) {
          assert(SharedDynamicArchivePath == NULL, "must be"); // FIXME test case
          no_shared_spaces("invalid archive");
        }
      }
