...
// automatically regenerate the archive as necessary (to be implemented in JDK-8261455).
...
public:
  ...
  bool initialize(int fd) {
    ...
    if (gen_header._magic != CDS_ARCHIVE_MAGIC && 
      ...
      FileMapInfo::fail_continue("The shared archive file has a bad magic number: %0xx", gen_header._magic);
      return false;
    }
  ...
  GenericCDSFileMapHeader* get_generic_file_header() {
    assert(_header != NULL && _is_valid, "must be a valid archive file");
    return _header;
  }

  const char* base_archive_name() {
    assert(_header != NULL && _is_valid, "must be a valid archive file");
    return _base_archive_name; // FIXME comment
  }
...
//      (*base_archive_name) is the static archive which <archive_name> depends on.
