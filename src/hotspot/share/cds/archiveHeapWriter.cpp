...
ArchiveHeapWriter::BufferOffsetToSourceObjectTable*
  ArchiveHeapWriter::_buffer_offset_to_source_obj_table = NULL;
...
bool ArchiveHeapWriter::is_in_requested_range(oop o) {
  assert(_requested_bottom != NULL, "do not call before _requested_bottom is initialized");
...
address ArchiveHeapWriter::requested_address() {
  assert(_buffer != NULL, "must be initialized");
...
void ArchiveHeapWriter::copy_roots_to_buffer(GrowableArrayCHeap<oop, mtClassShared>* roots) {
  ...
  int length = roots != NULL ? roots->length() : 0;
...
template <typename T> void ArchiveHeapWriter::relocate_field_in_requested_obj(oop requested_obj, size_t field_offset, CHeapBitMap* oopmap) {
  ...
  if (!CompressedOops::is_null(source_referent)) {
    ...
    mark_oop_pointer<T>(requested_obj, field_offset, oopmap);
  }
}

template <typename T> void ArchiveHeapWriter::mark_oop_pointer(oop requested_obj, size_t field_offset, CHeapBitMap* oopmap) {
  T* request_p = (T*)(cast_from_oop<address>(requested_obj) + field_offset);
...
template <typename T> void ArchiveHeapWriter::relocate_root_at(oop requested_roots, int index, CHeapBitMap* oopmap) {
  ...
  relocate_field_in_requested_obj<T>(requested_roots, offset, oopmap);
}
...
class ArchiveHeapWriter::EmbeddedOopRelocator: public BasicOopIterateClosure {
  ...
  CHeapBitMap* _oopmap;
public:
  EmbeddedOopRelocator(oop src_obj, oop requested_obj, CHeapBitMap* oopmap) :
    _src_obj(src_obj), _requested_obj(requested_obj), _oopmap(oopmap) {}
...
private:
  template <class T> void do_oop_work(T *p) {
    ...
    ArchiveHeapWriter::relocate_field_in_requested_obj<T>(_requested_obj, field_offset, _oopmap);
  }
};
...
void ArchiveHeapWriter::relocate_embedded_oops(GrowableArrayCHeap<oop, mtClassShared>* roots,
  ...
  auto iterator = [&] (oop src_obj, HeapShared::CachedOopInfo& info) {
    ...
    EmbeddedOopRelocator relocator(src_obj, requested_obj, heap_info->oopmap());
  ...
  update_header_for_requested_obj(requested_roots, NULL, Universe::objectArrayKlassObj());
  int length = roots != NULL ? roots->length() : 0;
  for (int i = 0; i < length; i++) {
    if (UseCompressedOops) {
