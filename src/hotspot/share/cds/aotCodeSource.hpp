...
public:
  ...
  bool check(const char* runtime_path) const;
};
...
class AOTCodeSourceConfig : public CHeapObj<mtClassShared> {
  ...
  bool check_classpaths(bool is_boot_classpath, int index_start, int index_end, CodeSourceStream& runtime_css,
                        bool use_lcp_match, const char* runtime_lcp, size_t runtime_lcp_len) const;
  bool check_module_paths(int index_start, int index_end, CodeSourceStream& runtime_css,
...
public:
  ...
  bool validate(bool* has_extra_module_paths) const;
};
