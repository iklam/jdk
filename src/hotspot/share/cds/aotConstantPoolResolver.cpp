...
static InstanceKlass* find_unsupported_interface_for_lambda(InstanceKlass* intf) {
  ...
  //
  // We don't support aot-linking of a lambda if initializing its interfaces can
  // cause side effects, as checked by this function.

  if (!intf->has_nonstatic_concrete_methods()) {
    // no nonstatic concrete methods are declared by intf or its super interfaces
    return nullptr;
  }

  if (intf->declares_nonstatic_concrete_methods() && intf->class_initializer() != nullptr) {
    return intf;
  }

  for (int i = 0; i < intf->transitive_interfaces()->length(); ++i) {
    InstanceKlass* super_intf = intf->transitive_interfaces()->at(i);
  ...
  for (SignatureStream ss(sig); !ss.is_done(); ss.next()) {
    if (ss.is_reference()) {
      ...
      if (check_return_type && ss.at_return_type()) {
        ...
        InstanceKlass* unsupported_intf = find_unsupported_interface_for_lambda(intf);
        if (log_is_enabled(Debug, cds, resolve)) {
          ResourceMark rm;
          if (unsupported_intf != nullptr) {
            bool is_super = (unsupported_intf != intf);
            log_debug(cds, resolve)("Cannot aot-resolve Lambda proxy of interface type %s (unsupported %s%s)",
                                    k->external_name(),
                                    is_super ? "super interface " : "interface",
                                    is_super ? unsupported_intf->external_name() : "");
          } else {
            log_debug(cds, resolve)("Can aot-resolve Lambda proxy of interface type %s", k->external_name());
          }
        }
        if (unsupported_intf != nullptr) {
          return false;
