...
AOTCodeSource* AOTCodeSource::allocate(JavaThread* current, const char* path, int index,
  ...
  cs->_check_time_size = check_time_size;
...
bool AOTCodeSource::check(const char* runtime_path) const {
  struct stat st;
  if (os::stat(runtime_path, &st) != 0) {
    ...
    bool time_differs = _timestamp != st.st_mtime;
...
bool AOTCodeSourceConfig::check_classpaths(bool is_boot_classpath, int index_start, int index_end,
  ...
  for (int i = index_start; i < index_end; i++) {
    ...
    if (!cs->check(effective_dumptime_path)) {
      return false;
    }
  }
...
bool AOTCodeSourceConfig::check_module_paths(int index_start, int index_end,
                                             CodeSourceStream& runtime_css,
...
bool AOTCodeSourceConfig::validate(bool* has_extra_module_paths) const {
  ResourceMark rm;
  AllCodeSourceStreams all_css;

  const char* jrt = ClassLoader::get_jrt_entry()->name();
  bool success = code_source_at(0)->check(jrt);
  ...
  if (code_sources()->length() == 1) {
    *has_extra_module_paths = false;
    ...
    success = check_classpaths(true, boot_start(), boot_end(), all_css.boot_cp(),
    ...
    if (success && need_to_check_app_classpath()) {
      success = check_classpaths(false, app_start(), app_end(), all_css.app_cp(),
                                 use_lcp_match, runtime_lcp, runtime_lcp_len);
      log_info(class, path)("Archived app classpath validation: %s", success ? "passed" : "failed");
    }

    if (success) {
      success = check_module_paths(module_start(), module_end(), all_css.module_path(), has_extra_module_paths);
...
[1] dumptime: -bootclasspath/a:A.jar
    runtime:  -bootclasspath/a:A.jar:B.jar

[2] Is the current behavior correct???

    $ ls -l HelloWorld.class
    -rw-r--r-- 1 iklam iklam 425 Jan  7 15:02 HelloWorld.class
    $ cat > classlist
    HelloWorld
    $ java -cp . -Xshare:dump -XX:SharedClassListFile=classlist
    [1.124s][warning][cds] Skipping HelloWorld: Unsupported location
    $ java -cp . -Xshare:on HelloWorld
    Hello World


[3] non-empty dir inside "Class-Path:" attribute. Should we abort dumping?

[4] "." is inside "Class-Path:" attribute, and contains HelloWorld.class.
    Should we abort dumping?

[5] dumptime: -cp <non-exist>:HelloWorld.jar
    runtime:  -cp HelloWorld.jar
