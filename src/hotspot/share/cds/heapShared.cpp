...
int HeapShared::find_root_index_for_archived_object(oop orig_obj) {
  assert(DumpSharedSpaces, "dump-time only");
  ArchivedObjectCache* cache = archived_object_cache();
  CachedOopInfo* p = cache->get(orig_obj);
  assert(p != NULL, "must be already archived");
  return append_root(orig_obj);
}
...
bool HeapShared::archive_object(oop obj) {
  ...
  if (ArchiveHeapWriter::is_too_large_to_archive(obj->size())) {
    ...
    if (java_lang_Module::is_instance(obj)) {
      check_module_oop(obj);
      java_lang_Module::set_module_entry(obj, NULL);
      ...
      guarantee(obj == SystemDictionary::java_platform_loader() ||
                ...
                java_lang_ClassLoader::loader_data(obj) == NULL, "must be");
      java_lang_ClassLoader::release_set_loader_data(obj, NULL);
    }

    count_allocation(obj->size());
    ArchiveHeapWriter::add_source_obj(obj);
    obj->identity_hash(); // Need to call this in a deterministic order
